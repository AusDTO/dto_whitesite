...
function dto_whitesite_preprocess_maintenance_page(&$variables, $hook) {
  // When a variable is manipulated or added in preprocess_html or
  // preprocess_page, that same work is probably needed for the maintenance page
  // as well, so we can just re-use those functions to do that work here.
  dto_whitesite_preprocess_html($variables, $hook);
  dto_whitesite_preprocess_page($variables, $hook);
}
// */
...
function dto_whitesite_preprocess_page(&$variables, $hook) {
  ...
  if (!empty($vars['page']['content']['system_main']['comment_node']['#node'])) {
    $node = $vars['page']['content']['system_main']['comment_node']['#node'];
    $vars['node_title'] = check_plain($node->title);
  }
}
 ...
 */
/* -- Delete this line if you want to use this function
function dto_whitesite_preprocess_node(&$variables, $hook) {
  $variables['sample_variable'] = t('Lorem ipsum.');
 ...
 */
/* -- Delete this line if you want to use this function
function dto_whitesite_preprocess_comment(&$variables, $hook) {
  $variables['sample_variable'] = t('Lorem ipsum.');
}
// */
 ...
 */
/* -- Delete this line if you want to use this function
function dto_whitesite_preprocess_region(&$variables, $hook) {
 ...
 */
/* -- Delete this line if you want to use this function
function dto_whitesite_preprocess_block(&$variables, $hook) {
...
function dto_whitesite_agov_social_link($variables) {
  ...
  if ($title == 'Youtube') {
    $variables['icon'] = base_path() . drupal_get_path('theme', 'dto_whitesite') . '/images/youtube.png';
  }
 ...
 */
function dto_whitesite_button($variables) {
 ...
 */
function dto_whitesite_pager($variables) {
  $tags = $variables['tags'];
  $element = $variables['element'];
 ...
 */
function dto_whitesite_pager_link__responsive($variables) {
 ...
 */
function dto_whitesite_pager_first__responsive($variables) {
  $text = $variables['text'];
  $element = $variables['element'];
  $parameters = $variables['parameters'];
 ...
 */
function dto_whitesite_pager_previous__responsive($variables) {
  $text = $variables['text'];
  $element = $variables['element'];
 ...
 */
function dto_whitesite_pager_next__responsive($variables) {
  $text = $variables['text'];
  $element = $variables['element'];
  $interval = $variables['interval'];
  $parameters = $variables['parameters'];
 ...
 */
function dto_whitesite_pager_last__responsive($variables) {
  $text = $variables['text'];
  $element = $variables['element'];
  $parameters = $variables['parameters'];
  global $pager_page_array, $pager_total;
 ...
 */
function dto_whitesite_item_list__pager($variables) {
  ...
  if (!empty($items)) {
    ...
    foreach ($items as $item) {
      ...
      if (count($children) > 0) {
        // Render nested list.
        $data .= dto_whitesite_item_list__pager(array(
          'items' => $children,
          'attributes' => array(),
        ));
      }
 ...
 */
function dto_whitesite_preprocess_menu_link(&$variables, $hook) {
  ...
  if (isset($variables['element']['#bid']['delta']) && $variables['element']['#bid']['delta'] == 'ctools-menu-frontpage-quicklinks-1') {
    ...
    if ($svg) {
      $svg = file_get_contents(DRUPAL_ROOT . '/' . drupal_get_path('theme', 'dto_whitesite') . '/images/icons/quick-links--' . $svg . '.svg');
    }
